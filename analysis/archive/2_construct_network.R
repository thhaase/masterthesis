...
raw <- raw |> 
  mutate(id = as.character(id)) |> setDT()
...
threadinfo[d, on = .(id_root = id), `:=`(
  root_user_name_d = i.user_name,
  root_user_screen_name_d = i.user_screen_name,
  root_party_d = i.party
)]

threadinfo$root_party_d |> table()

threadinfo[raw, on = .(id_root = id), `:=`(
  root_user_name_raw = i.user_name,
  root_user_screen_name_raw = i.user_screen_name,
  root_party_raw = i.party
)]

threadinfo$root_party_raw |> table()
...
temp_pol <- d[!is.na(user_name), .(user_name, user_description, party)] |> unique()

d[,c("id", "user_name", "user_screen_name", "party")]
