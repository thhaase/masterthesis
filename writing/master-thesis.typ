...
*/
#let long-caption(width: 85%, body) = align(center)[
  #block(width: width)[
    #text(size: 10pt)[#body]
  ]
]
...
After introducing the dataset and collection procedure the three step analysis is described. 
...
Second, the construction of reply threads, the user-level reply network and the exploration of their structure are detailed. 
Finally in a synthesis, politician ego networks are combined with the populism scores produced by the text analysis. Linear Regressions with their controls, as well as a robustness check and further exploration of the ego networks is described.
...
The dataset used in this study was kindly provided by Dr. Armin Pournaki from the Max Planck Institute for Mathematics in the Sciences, Leipzig. 
//#text(fill: red)[did armin use this data somewhere in his own studies?? maybe I can cite something] 
The dataset holds tweets and replies directed at German members of parliament as well as their retweets and referenced tweets, collected over a one week from February 7 to February 14, 2022. Only the MPs tweets and their replies are analysed since the reply network best captures social interactions of engagement communities. Data were obtained via Twitter's streaming API using two parallel strategies. The follow stream tracked retweets of and replies to MP-generated tweets, while the track stream captured tweets mentioning MP handles (e.g., #quote[\@username]), including direct and nested replies. Protected accounts were excluded. Both datasets were combined and deduplicated. From the raw data, reply-threads were constructed by linking tweets through directed reply chains. These interactions were then aggregated to a user-level reply network, weighted by the number of exchanges. Thus, the dataset enables an analysis of interaction patterns between politicians and their engagement communities on social media.
To deliniate the set of politicians and add their name and party information, a list of parliamentary members of the 19th, 20th, and 21st German Bundestag is obtained from the Bundestags Webarchiv and added to the original dataset through their twitter-account links @bundestag2026. This ensures that the analysis accounts for politicians active in the legislative periods preceding, during, and following the timeframe of the collected tweets. 
The complete dataset consists of 693 015 Tweets in the twitwi format @medialab2026.
...
//#lorem(500) +143words
Current studies almost uniformly base their understanding of populism on #cite(<mudde2004>,form: "prose")'s definition of populism as #quote[two homogeneous and antagonistic groups, ‘the pure people’ versus ‘the corrupt elite’, and which argues that politics should be an expression of the volonté générale (general will) of the people.] @mudde2004[542]. While all operationalizations include a #quote[pro-people] and #quote[anti-elite] one of multiple third dimensions is often implemented as well like the inclusion of anti-pluralist attitudes, the distinction between leftwing and rightwing populism or agitating against horizontal outgroups like minorities @aalberg2017 @castanhosilva2020 @meyer2025. Populist Attitudes are not only measured through surveys @castanhosilva2020, but also through observational studies of political discourse on social media @meyer2025 leveraging LLMs.  
...
At each step, the model is asked to consider alternative readings and flag its confidence as #quote[low] when a reasonable coder could disagree. 
...
The model also implements expert segmentation, is multilingual and has 8-bit floating weights @yang2025. To run the model most efficiently on the HPC Capella Cluster of the TU-Dresden the vLLM framework was used through python to make use of virtual memory PagedAttention techniques @kwon2023. 

Output quality of large language models is highly dependent on prompt design @liu2026. Prompt engineering is conducted iteratively by drawing small random samples of tweets from the corpus and classifying it using an initial prompt. The resulting reasoning steps contained in the LLMs JSON output, like the actor analysis for example, are then qualitatively assessed with regard to coherence and later label accuracy. Based on identified shortcomings, the prompt is systematically adjusted and the process is repeated. 
Early versions of the prompt were structured in the style described by #cite(<liu2026>, form:"prose"). Later additional rule based checks were integrated and the coherence of the phrasing was adjusted. The rule based reasoning guidence was replaced for the final prompt with reflective questions prompting the LLM to question its judgements and certainty. This was related to a large increase in reasoning and output performance. These adjustments were done with a tiny subsample of the dataset.
External validity is tested after arriving at a sufficiently well performing prompt. 
The expert annotated training dataset of Bundestag speech sentences, used to fine tune the PopBERT populism classification transformer, serves as a final validation of the prompt @erhard2025. The dataset contains 8795 sentences of Bundestag speeches with binary annotations of five experts on the people-centrism and anti-elite dimension @erhard2025. After annotating the validation dataset locally with the developed systemprompt and Qwen3-235B-A22B-Instruct-2507-FP8 accuracy 
...
for pairs of expert raters with each other and the LLM annotations are calculated. For validation the bidirectional rating scale of the LLM is collapsed to binary labels matching the experts labels of pro-people and anti-elite. Pairwise calculations are possible because the F1 score is not symmetric for swapping labels but it is symmetric for swapping raters (see #link(<sec:app-symmetry-of-f1-score>)[Appendix]).
...
The network analysis is based on the giant component to make computation more feasible leading to a loss of the network periphery without any actual structurally relevant threads. 
...
A descriptive exploratory analysis describes first the results of the textanalysis and second the networkstructure. Following that, first the most relevant descriptive network statistics are calculated. Additionally the reply network structure is explored through a visualization using the Distributed Recursive Graph Layout @martin2007.  
...
Populism seems to be a fuzzy concept where even expert raters have a hard time deciding on the strength and presence on the anti-elitism dimension but agree even less on the people-centrism dimension. 
...
#long-caption[
  Nodes colored by the correlation of the original tweet with respective populism score; Network laid out with closeness-constrained radial layout @brandes2011. Nodes with similar shortest average distance are placed on the same circle and face the same angle by sharing common neighbors. 4024 tweets from non-populist MPs, 2275 tweets from populist MPs.
]
...
#long-caption[MPs populism score > 0; MPs populism score < 0. 
  Nodes colored by the correlation of the original tweet with respective populism score; Network laid out with strength-constrained radial layout @brandes2011. Nodes share a circle when their degree of weighted edges is comparable, and share an angular sector when they lie in the same densely connected region of the graph. 
]
...
The analysis now turns to the structure of the reply network as the relational substrate on which politicians and citizens meet. @tab:network-descriptives summarizes this network as a directed weighted network anchored in politicians' threads as described in @sec:method-networkanalysis.
...
#figure(
  image("../images/3-degree-distribution.png", width: 80%),
  caption: [In and outdegree distributions of the giant component on log-log scale]
)<fig:degree-distributions>
#long-caption[
  Frequency of users with a given in or outdegree in the giant component (n $= 68thin 240$). Indegree counts replies received per user, outdegree counts replies sent. Both axes log scaled. 
]
...
#figure(
  image("../images/3-coreness-x-nodes.png", width: 70%),
...
#figure(
  image("../images/network_inset.png", width: 121%),
  caption: [Giant Component of Reply Network]
)<fig:network-inset>
#long-caption[
  Reply Network laid out with the DrL Algorithm @martin2007. Nodesizes scale with degree, politicians are colored by their partycolor. The zoomed inset highlights the directed edges between alters in Alice Weidels' egonetwork.   
]

@fig:network-inset visualises the giant component of the reply network. The network is laid out with the Distributed Recursive Layout (DrL) algorithm @martin2007. At first a force directed network is laid out, which is then iteratively hierarchically clustered into fewer nodes until a sufficiently small network is created. In the end the finer graphs are drawn in again. 
...
#figure(
  image("../images/5-H1_egonet_models.png", width: 105%),
...
@fig:predicted-values visualizes the predicted means from the full model with the `prediction` function of the marginal effects package @arel-bundock2026. It visualizes the discrepancy of means and samplesizes between the populist and non-populist ego networks. The figure makes the impact of populist vocabulary tangible. Populist rated politicians produce almost three times as connected engagement-communities compared to non populist politicians. The limitations of the small sample size are clearly visible -- the few displayed populist egonetworks could very well be an $alpha$ (Type I) error and have occured by random chance. Additionally to the data being sparse a lot of politicians only get extremely few replies. 
...
- Egonetwork analysis
  ...
  - exploring alters contribution to the them forming connections through comparing attributes of alter-pairs that formed edges with random alter pairs of the respective egonetwork
    - Only difference between populist and non-populist networks is that in populist politicians engagementcommunities users of different tweetvolumes tend to connect more to each other, while in non-populist networks its the other way around or closer to just the random baseline
