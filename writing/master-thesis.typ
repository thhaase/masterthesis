...
// Introduction                     [1,110w, 2.1 pages, ~10%] -400 words
...
The analysis is based on a dataset of 693,015 tweets collected via Twitter's streaming API from 7-14 February 2022. Mudde's definition is operationalized along three dimensions — People Attitude, Elite Attitude, and Antagonism — scored by Qwen3-235B via a chain-of-thought prompt validated against the PopBERT expert corpus. Reply threads are reconstructed into a weighted user-level network whose giant component (77,194 nodes; 239,502 edges) forms the analytical base. Ego networks are extracted around politicians to focus on local structure. Two OLS models regress mean alter degree on populism, controlling for ego degree, follower count, and mean thread size; fragmentation ratio serves as a second dependent variable for robustness. To move beyond aggregated ego-network measures toward social mechanisms, alter-alter tie probability is visualized descriptively to compare engagement patterns in populist and non-populist communities. The results are then presented and discussed alongside their limitations.
